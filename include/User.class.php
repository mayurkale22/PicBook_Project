...
class PHPS_User {
	...
	public function user_login($email, $password, $javascript_disabled = 0, $persistent = 0) {
		...
		if($email == '') {
			$this->is_error = 1;
			$this->error_message = $Application[755];

		}elseif($password == '') {
			$this->is_error = 1;
			$this->error_message = $Application[756];

		}
		// show error if js is disabled
		elseif($javascript_disabled == 1) {
			$this->is_error = 1;
	...
	public function user_password($password_old, $password, $password_confirm, $check_old = 1) {
		global $Application;
		if(str_replace(" ", "", $password) == "" | str_replace(" ", "", $password_confirm) == "" | ($check_old == 1 & str_replace(" ", "", $password_old) == "")) 
			{ $this->is_error = 1; $this->error_message = $Application[38]; }
		if($check_old == 1 & crypt($password_old, $this->user_salt) != $this->user_info[user_password]) 
			{ $this->is_error = 1; $this->error_message = $Application[45]; }
		if($password != $password_confirm)
			{ $this->is_error = 1; $this->error_message = $Application[46]; }
		if(str_replace(" ", "", $password) != "" & strlen($password) < 6) 
			{ $this->is_error = 1; $this->error_message = $Application[48]; }
		if(ereg('[^A-Za-z0-9]', $password)) 
			{ $this->is_error = 1; $this->error_message = $Application[48]; }
		if (!preg_match("#[0-9]+#", $password)) 
			{ $this->is_error = 1; $this->error_message = $Application[49];}
	...
	public function user_friend_total($direction = 0, $friend_status = 1, $user_details = 0, $where = "") {
	  ...
	  if($setting[setting_connection_allow] != 0) {
	    ...
	    // begin query
	    $friend_query = "SELECT friend_id FROM friends";

	    // joing to friends table in needed
	    if($user_details == 1) { 
	      $friend_query .= " LEFT JOIN phps_users ON";
	      if($direction == 1) { $friend_query .= " friends.friend_user_id1=phps_users.user_id"; } else { $friend_query .= " friends.friend_user_id2=phps_users.user_id"; }
	    }
	...
	public function user_friend_list($start, $limit, $direction = 0, $friend_status = 1, $sort_by = "phps_users.user_dateupdated DESC", $where = "", $friend_details = 0) {
	  ...
	  if($setting[setting_connection_allow] != 0) {
	    ...
	    $friend_query = "SELECT friends.friend_id, phps_users.user_id, phps_users.user_phone, phps_users.user_username, phps_users.user_photo, phps_users.user_lastlogindate, phps_users.user_dateupdated";

	    $friend_query .= " FROM friends LEFT JOIN phps_users ON";
	...
	public function user_friend_add($other_user_id, $friend_status, $friend_type, $friend_explain) {
	  ...
	  // add to frinds
	  $database->database_query("INSERT INTO friends (friend_user_id1, friend_user_id2, friend_status) VALUES ('".$this->user_info[user_id]."', '$other_user_id', '$friend_status')");
	...
	public function user_friend_remove($other_user_id) {
	  global $database, $setting;
          ...
          $friend1 = $database->database_query("SELECT friend_id FROM friends WHERE friend_user_id1='".$this->user_info[user_id]."' AND friend_user_id2='$other_user_id'");
          if($database->database_num_rows($friend1) == 1) {
            $friendship = $database->database_fetch_assoc($friend1);
            $database->database_query("DELETE FROM friends WHERE friend_id='$friendship[friend_id]'");
            
          }

          // remove additional raw if two-derectionals
          $friend2 = $database->database_query("SELECT friend_id FROM friends WHERE friend_user_id2='".$this->user_info[user_id]."' AND friend_user_id1='$other_user_id'");
	...
	public function user_friend_of_friend($other_user_id) {
	  ...
	  if($database->database_num_rows($database->database_query("SELECT t2.friend_user_id2 FROM friends AS t1 LEFT JOIN friends AS t2 ON t1.friend_user_id2=t2.friend_user_id1 WHERE t1.friend_user_id1='".$this->user_info[user_id]."' AND t2.friend_user_id2='$other_user_id' AND t1.friend_status<>'0' AND t2.friend_status<>'0'")) != 0) {
	    return true;
	  } else {
	    return false;
	  }
	...
	public function user_friended($other_user_id, $friend_status = 1) {
	  ...
	  if($database->database_num_rows($database->database_query("SELECT friend_id FROM friends WHERE friend_user_id1='".$this->user_info[user_id]."' AND friend_user_id2='$other_user_id' AND friend_status='$friend_status'")) == 1) {
	    return true;
	  } else {
	    return false;
	  }
