...
if($task == "remove" | $task == "edit" | $task == "editdo") {
  $friend = $database->database_query("SELECT friend_id FROM friends WHERE friend_user_id1='".$user->user_info[user_id]."' AND friend_user_id2='".$owner->user_info[user_id]."' AND friend_status='1'");
  if($database->database_num_rows($friend) != 1) { header("Location: UserFriends.php"); exit(); }
} elseif($task == "cancelrequest") {
  $friend = $database->database_query("SELECT friend_id FROM friends WHERE friend_user_id1='".$user->user_info[user_id]."' AND friend_user_id2='".$owner->user_info[user_id]."' AND friend_status='0'");
  if($database->database_num_rows($friend) != 1) { header("Location: UserFriendsRequestsOutgoing.php"); exit(); }
} else {
  $friend = $database->database_query("SELECT friend_id FROM friends WHERE friend_user_id1='".$owner->user_info[user_id]."' AND friend_user_id2='".$user->user_info[user_id]."' AND friend_status='0'");
...
if($task == "confirm") {
  ...
  $friend1 = $database->database_query("SELECT friend_id FROM friends WHERE friend_user_id1='".$owner->user_info[user_id]."' AND friend_user_id2='".$user->user_info[user_id]."' AND friend_status='0'");
  $friend2 = $database->database_query("SELECT friend_id FROM friends WHERE friend_user_id2='".$owner->user_info[user_id]."' AND friend_user_id1='".$user->user_info[user_id]."'");

  // CONFIRM FRIEND REQUEST  
  if($database->database_num_rows($friend1) == 1) {
    $friendship = $database->database_fetch_assoc($friend1);
    $database->database_query("UPDATE friends SET friend_status='1' WHERE friend_id='$friendship[friend_id]'");
...
if($task == "editdo") {
  ...
  $friendship = $database->database_fetch_assoc($database->database_query("SELECT friend_id FROM friends WHERE friend_user_id2='".$owner->user_info[user_id]."' AND friend_user_id1='".$user->user_info[user_id]."' AND friend_status='1'"));

  $database->database_query("UPDATE friends SET friend_type='$friend_type' WHERE friend_id='$friendship[friend_id]'");
...
if($task == "edit") {
  ...
  $friendship = $database->database_fetch_assoc($database->database_query("SELECT friend_id, friend_type FROM friends WHERE friend_user_id2='".$owner->user_info[user_id]."' AND friend_user_id1='".$user->user_info[user_id]."' AND friend_status='1'"));
