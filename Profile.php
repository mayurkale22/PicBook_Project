...
$actions_total = count($actions);
	...
	$user2_query = "SELECT user_id FROM phps_users WHERE user_username ='".$_GET['user']."'";
	$user2 = $database->database_fetch_assoc($database->database_query($user2_query));


$is_friend = 0;
if($user->user_info['user_id'] == $user2['user_id'])
	{
		{$is_friend = 1;}
	}
	else
	{
		$friendcheck_query = "SELECT friend_id FROM friends WHERE friend_user_id1 ='".$user->user_info['user_id']."' 
							  and friend_user_id2 ='".$user2['user_id']."' And friend_status=1";
		if( $database->database_num_rows($database->database_query($friendcheck_query)) == 1){$is_friend = 1;}
	}
	$board_idarray = Array();
	$b_query = "SELECT board_id FROM boards b, phps_users u WHERE u.user_id = b.user_id and u.user_username ='".$_GET['user']."'";
	if ($is_friend==0) {$b_query .="  and b.is_visible='1'";}
	
	$boards_query = $database->database_query($b_query); 
	
	while($boards_info = $database->database_fetch_assoc($boards_query)) {
	$board_idarray[]=$boards_info[board_id];
	}
	$board_cnt = 0;
	$bcnt_query = "SELECT board_id FROM boards b , phps_users u WHERE u.user_id = b.user_id and u.user_username ='".$_GET['user']."'";
	if ($is_friend==0) {$bcnt_query .="  and b.is_visible='1'";}
		
	$b_cnt = $database->database_query($bcnt_query); 
